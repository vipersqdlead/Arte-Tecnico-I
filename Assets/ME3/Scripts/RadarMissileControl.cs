    ...
    void FireMissile()
    {
        if (MissileAmmo > 0)
        {
            int currentIndex = MissileAmmo - 1;
            {
                ...
                missilerb.AddForce(gameObject.GetComponent<Rigidbody>().linearVelocity, ForceMode.VelocityChange);
                missilerb.rotation = Quaternion.LookRotation(gameObject.GetComponent<Rigidbody>().linearVelocity);
